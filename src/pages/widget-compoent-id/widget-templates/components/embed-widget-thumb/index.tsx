...
import { EmbedWidgetThumbInterface } from "../../../../../interface/embed-widget-thumb-interface";
  ...
  return (
    ...
    >
      ...
      >
        {/*Image*/}
        <div
          className={`${style.embedWidgetThumbImageContainer} ${selected && style.beforeClassPlayNows}`}
          aria-label={item?.name}
        >
          <Image
            data-testid="close-icon"
            aria-label={`Thumbnail Name": ${item?.name}`}
            className={style.embedWidgetThumbImageImage}
            src={item?.thumbnailUrl || "/assets/arrow-left-p.svg"}
            alt={item?.name}
            height="500"
            width="500"
          />
      ...
      {/*Name*/}
      <div
        className={style.embedWidgetThumbText}
        style={{ order: "1", color: thumbnailTitleColor }}
        aria-label={`Thumbnail Name": ${item?.name}`}
      >
        {item?.name}
      </div>
